...
# Lineout parameters
target_y = 0.8382  # Fixed axial height in meters
...
df_ref.columns = ['Axial velocity', 'x_D']
...
plt.plot(df_ref['Axial velocity'], df_ref['x_D'], 
...
# Formatting
plt.xlabel('Axial velocity [m/s]', fontsize=12)
plt.ylabel('x / D', fontsize=12)
#plt.title(f'Axial velocity Profile at y = {target_y} m\n(Time-Average across {len(plotfiles)} snapshots)')
...
plt.tight_layout()
plt.savefig('radial_comparison_avg.png', dpi=300)
plt.show()