...
def plot_temp_and_heat_release(plt_path, output_folder):
    ...
    try:
        ...
        # --- Plot CH4 (Right Panel) ---
        slc_hr = yt.SlicePlot(ds, "z", ("boxlib", "Y(CH4)"))
        frb_hr = slc_hr.frb
        
        hr = frb_hr[("boxlib", "Y(CH4)")].v
        ...
        fig.colorbar(im2, ax=ax2, label="Y(CH4)")
        ax2.set_xlabel("x (m)")
        ax2.set_ylabel("y (m)")
        
        # Save with unique name
        plt_num = os.path.basename(plt_path)
        output_path = os.path.join(output_folder, f"{plt_num}_temp_ch4_mesh.png")
